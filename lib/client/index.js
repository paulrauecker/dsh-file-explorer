...
const ExplorerPanel = (props) => {
    ...
    const onSave = () => {
        ...
        api.write(path, content).then((res) => {
            if (res && res.error)
                showStatus({ ok: false, text: '保存失败：' + res.error });
            else {
    ...
    const renderTree = () => {
        if (!tree || !tree.rootPath)
            return react.createElement('div', { className: 'fe-empty' }, '未找到当前工作区');
        const rows = [];
    ...
    const renderSearch = () => {
        if (s.searching && !s.matches)
            return react.createElement('div', { className: 'fe-empty' }, '搜索中…');
        if (s.searchError)
            return react.createElement('div', { className: 'fe-node-error' }, s.searchError);
        if (!s.matches || s.matches.length === 0)
            return react.createElement('div', { className: 'fe-empty' }, '没有匹配的文件');
        const rows = [];
        ...
        if (s.truncated)
            rows.push(react.createElement('div', { key: 'trunc', className: 'fe-node-error' }, '结果过多，已截断（前 300 条）'));
        return rows;
    };
    ...
    const renderPreview = () => {
        ...
        const head = react.createElement('div', { className: 'fe-editor-head' }, react.createElement('span', { className: 'fe-editor-name', title: editor.name }, editor.name), react.createElement('span', { className: 'fe-editor-path' }, editor.path), isMd && editor.state === 'ready' && !editor.editing
            ...
            : null, react.createElement('button', { className: 'fe-btn', onClick: () => { closePreviewPane(); setStatus(null); } }, '收起预览'));
        let body = null;
        if (editor.state === 'loading')
            body = react.createElement('div', { className: 'fe-editor-msg' }, '加载中…');
            ...
            body = react.createElement('div', { className: 'fe-editor-msg' }, '文件过大（' + fmtSize(editor.size) + '），不支持预览');
        else if (showPreview)
    ...
    const treePanel = s.open ? react.createElement('div', { className: 'fe-panel', style: { width: s.width + 'px' } }, react.createElement('div', { className: 'fe-resize', title: '拖动调整宽度', onPointerDown: (e) => onResizeStart('tree', e) }), treeTab, react.createElement('div', { className: 'fe-header' }, react.createElement('span', { className: 'fe-title' }, '文件'), react.createElement('button', { className: 'fe-iconbtn fe-icon-vscode', title: '在 Visual Studio Code 中打开项目', onClick: onVscode }, react.createElement(Icon, { name: 'vscode', size: 15 })), react.createElement('button', { className: 'fe-iconbtn', title: (tree && tree.selected) ? '在系统文件浏览器中定位选中项' : '在系统文件浏览器中打开项目', onClick: onOpenFolder }, react.createElement(Icon, { name: 'openInNew', size: 15 })), react.createElement('button', { className: 'fe-iconbtn', title: '全部展开 / 全部折叠', onClick: toggleAll }, react.createElement(Icon, { name: 'chevronDown', size: 14 })), react.createElement('button', { className: 'fe-iconbtn', title: '刷新', onClick: refresh }, react.createElement(Icon, { name: 'refresh', size: 14 })), react.createElement('button', { className: 'fe-iconbtn' + (editor && editor.editing ? ' fe-iconbtn-on' : ''), title: '编辑', onClick: onEditClick }, react.createElement(Icon, { name: 'edit', size: 14 })), react.createElement('button', { className: 'fe-iconbtn', title: '收起文件树', onClick: () => setOpen(false) }, react.createElement(Icon, { name: 'close', size: 14 }))), react.createElement('div', { className: 'fe-searchbar' }, react.createElement('input', { className: 'fe-search', type: 'text', placeholder: '搜索文件', value: s.query, spellCheck: false, onChange: (e) => setQuery(e.target.value) }), s.searching ? react.createElement('span', { className: 'fe-search-state' }, '…') : null), status ? react.createElement('div', { className: 'fe-status ' + (status.ok ? 'fe-status-ok' : 'fe-status-err') }, status.text) : null, react.createElement('div', { className: 'fe-tree' }, s.query.trim() ? renderSearch() : renderTree())) : null;
    const previewPane = editor
        ...
        }, react.createElement('div', { className: 'fe-preview-resize', title: '拖动调整宽度', onPointerDown: (e) => onResizeStart('preview', e) }), previewTab, renderTabBar(), renderPreview())
        : null;
...
function apply(ctx) {
    ...
    if (slots === undefined)
        return;
    slots.inject('shell.overlay', () => slots.register({ name: 'shell.overlay', id: 'file-explorer', order: 90, label: '文件资源管理器' }, (props) => react.createElement(ExplorerPanel, props)));
    slots.inject('conversation.session.header.actions', () => slots.register({ name: 'conversation.session.header.actions', id: 'file-explorer-toggle', order: 30, label: '文件资源管理器' }, (props) => react.createElement(ToggleButton, props)));
}
export { apply, inject };
