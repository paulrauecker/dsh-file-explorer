		...
		const ExplorerPanel = (props) => {
			...
			const openFile = (entry, startEditing, toggle) => {
				...
				if (existing) {
					...
					if (existing.editing) { clearPreviewToggle(); return; }
					if (startEditing) {
						clearPreviewToggle();
				...
				api.read(entry.path).then((res) => {
					updateTab(entry.path, (t) => {
						if (res && res.error) return { ...t, state: 'error', message: res.error, editing: false, preview: false };
						if (res && res.tooLarge) return { ...t, state: 'too-large', size: res.size, editing: false, preview: false };
