		...
		const ExplorerPanel = (props) => {
			...
			const onSave = () => {
				...
				api.write(path, content).then((res) => {
					...
					ok: false,
					text: "保存失败：" + String(err && err.message || err)
				}));
			};
			const refresh = () => {
			...
			const onVscode = () => {
				...
				api.openVscode(tree.rootPath).then((res) => {
					showStatus(res && res.ok ? {
						ok: true,
						text: "已在 VS Code 中打开项目"
					} : {
						ok: false,
						text: res && res.error || "打开失败（未检测到 code 命令）"
					});
				}).catch((err) => showStatus({
					ok: false,
					text: "打开失败：" + String(err && err.message || err)
				}));
			};
			const onOpenFolder = () => {
				if (!tree || !tree.rootPath) return;
				...
				api.openFolder(target).then((res) => {
					showStatus(res && res.ok ? {
						ok: true,
						text: tree.selected ? "已在系统文件浏览器中定位选中项" : "已在系统文件浏览器中打开项目"
					} : {
						ok: false,
						text: res && res.error || "打开失败"
					});
				}).catch((err) => showStatus({
					ok: false,
					text: "打开失败：" + String(err && err.message || err)
			...
			const renderPreview = () => {
				...
				}, editor.preview ? "源码" : "预览") : null, editor.state === "ready" && editor.editing ? react.createElement("button", {
					className: "fe-btn",
					onClick: onSave
				}, "保存") : null, react.createElement("button", {
					className: "fe-btn",
					onClick: () => {
						closePreviewPane();
						setStatus(null);
			...
			const treePanel = s.open ? react.createElement("div", {
			...
			}), treeTab, react.createElement("div", { className: "fe-header" }, react.createElement("span", { className: "fe-title" }, "文件"), react.createElement("button", {
				className: "fe-iconbtn fe-icon-vscode",
				title: "在 Visual Studio Code 中打开项目",
				onClick: onVscode
			}, react.createElement(Icon, {
				name: "vscode",
				size: 15
				...
				title: tree && tree.selected ? "在系统文件浏览器中定位选中项" : "在系统文件浏览器中打开项目",
			...
			}), s.searching ? react.createElement("span", { className: "fe-search-state" }, "…") : null), status ? react.createElement("div", { className: "fe-status " + (status.ok ? "fe-status-ok" : "fe-status-err") }, status.text) : null, react.createElement("div", { className: "fe-tree" }, s.query.trim() ? renderSearch() : renderTree())) : null;
