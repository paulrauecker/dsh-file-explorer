		...
		const ExplorerPanel = (props) => {
			...
			react.useEffect(() => {
				...
				if (s.open) root.setAttribute('data-fe-panel-open', '');
					...
					// Closing the sidebar clears the preview pane, so a fresh
					// open shows only the file tree. An in-progress edit
					// session is kept so unsaved changes are not lost.
					setEditor((e) => (e && e.editing ? e : null));
				}
				return () => {
					root.removeAttribute('data-fe-panel-open');
				};
