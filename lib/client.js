...
`;
		...
		const store = {
			...
			matches: null,
			truncated: false,
			// Preview pane and status message live in the shared store so
			// outer listeners (the chat double-click handler) can close them.
			editor: null,
			status: null,
			listeners: new Set(),
		};
		...
		// setEditor accepts an object or an updater function (React-style).
		const setEditor = (value) => { store.editor = typeof value === 'function' ? value(store.editor) : value; emit() };
		...
		const ExplorerPanel = (props) => {
			...
			react.useEffect(() => {
				...
				if (!rootPath) { setTree(null); resetTransient(); return }
			...
			react.useEffect(() => {
				...
				if (s.open || s.editor) root.setAttribute('data-fe-panel-open', '');
			...
			}, [s.width, previewWidth, s.open, editor ? 1 : 0]);
			...
			const previewTab = react.createElement('button', {
				...
				title: '收起文件预览',
				'aria-label': '收起文件预览',
				onClick: () => { setEditor(null); setStatus(null) },
			}, react.createElement(Icon, { name: 'chevronRight', size: 14 }));
		...
		function apply(ctx) {
			...
			// tree and the preview pane. Editable fields and interactive
			...
			const onChatDblClick = (e) => {
				if (!store.open && !store.editor) return;
