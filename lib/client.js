...
`;
		...
		const TreeNode = (props) => {
			...
			const nodes = [react.createElement("div", {
				...
				size: 12
			}) : null), react.createElement("span", { className: "fe-node-icon fe-node-" + (isDir ? "dir" : "file") }, react.createElement(Icon, {
			...
			if (isDir && expanded) {
				if (children) for (const child of children) nodes.push(react.createElement(TreeNode, {
					key: child.path,
					entry: child,
					depth: props.depth + 1,
					tree,
		...
		const ExplorerPanel = (props) => {
			...
			const hostErrorStatus = (res) => {
				if (res && res.code) return {
					ok: false,
					key: "error." + res.code,
					params: res.code === "open-exit-code" ? { code: res.exitCode } : void 0
				};
			...
			if (!rootPath && recentWorkspaceId) {
				for (const w of wsItems) if (w.workspaceId === recentWorkspaceId) {
					rootPath = w.path;
					rootName = w.title;
					break;
				}
			...
			const onSave = () => {
				...
				api.write(path, content).then((res) => {
					if (res && res.error) showStatus({
						ok: false,
						key: "error.saveFailed",
						params: { reason: res.error }
					});
					else {
						updateTab(path, (t) => ({
							...t,
					...
					ok: false,
					key: "error.saveFailed",
					params: { reason: String(err && err.message || err) }
				}));
			};
			const refresh = () => {
				...
				setTree({
					rootPath: root,
					rootName: name,
					expanded: /* @__PURE__ */ new Set([root]),
					cache: /* @__PURE__ */ new Map(),
					loading: /* @__PURE__ */ new Set([root]),
					selected: null,
					errors: {}
				});
				loadChildren(root);
			...
			const expandAll = () => {
				...
				work(tree.rootPath).then(() => {
					...
					if (loaded >= MAX_EXPAND_DIRS) showStatus({
						ok: false,
						key: "status.expandLimit",
						params: { count: MAX_EXPAND_DIRS }
					});
				}).catch(() => {
					expandBusy = false;
				});
			...
			const onVscode = () => {
				...
				api.openVscode(tree.rootPath).then((res) => {
					if (res && res.ok) showStatus({
						ok: true,
						key: "status.openedVscode"
					});
					else showStatus(hostErrorStatus(res));
				}).catch((err) => showStatus({
					ok: false,
					key: "error.openFailed",
					params: { reason: String(err && err.message || err) }
				}));
			};
			const onOpenFolder = () => {
				...
				api.openFolder(target).then((res) => {
					if (res && res.ok) showStatus({
						ok: true,
						key: tree.selected ? "status.revealedInFolder" : "status.openedFolder"
					});
					else showStatus(hostErrorStatus(res));
				}).catch((err) => showStatus({
					ok: false,
			...
			const renderTree = () => {
				...
				rows.push(react.createElement("div", {
					...
					size: 12
				})), react.createElement("span", { className: "fe-node-icon fe-node-dir" }, react.createElement(Icon, {
					name: "folder",
					size: 14
				})), react.createElement("span", {
					className: "fe-node-name",
				...
				if (tree.expanded.has(tree.rootPath)) {
					...
					if (children) for (const child of children) rows.push(react.createElement(TreeNode, {
						key: child.path,
						entry: child,
						depth: 1,
						tree,
			...
			const renderSearch = () => {
				...
				for (const m of s.matches) {
					...
					rows.push(react.createElement("div", {
						...
						title: m.path
					}, react.createElement("span", { className: "fe-node-icon fe-node-" + (m.type === "directory" ? "dir" : "file") }, react.createElement(Icon, {
						name: m.type === "directory" ? "folder" : "file",
						size: 14
					})), react.createElement("span", {
						className: "fe-node-name",
						title: m.name
				...
				if (s.truncated) rows.push(react.createElement("div", {
					key: "trunc",
					className: "fe-node-error"
				}, t("search.truncated", { count: SEARCH_TRUNC_LIMIT })));
				return rows;
			};
			const renderTabBar = () => {
			...
			const renderPreview = () => {
				...
				}, editor.preview ? t("action.viewSource") : t("action.viewPreview")) : null, editor.state === "ready" && editor.editing ? react.createElement("button", {
					className: "fe-btn",
					onClick: onSave
				}, t("action.save")) : null, react.createElement("button", {
					className: "fe-btn",
					onClick: () => {
						closePreviewPane();
						setStatus(null);
					}
				}, t("action.collapsePreview")));
				let body = null;
			...
			const treePanel = s.open ? react.createElement("div", {
			...
			}), treeTab, react.createElement("div", { className: "fe-header" }, react.createElement("span", { className: "fe-title" }, t("panel.filesLabel")), react.createElement("button", {
				className: "fe-iconbtn fe-icon-vscode",
				title: t("action.openInVscode"),
				onClick: onVscode
			}, react.createElement(Icon, {
				name: "vscode",
				size: 15
				...
				title: tree && tree.selected ? t("action.revealInFileManager") : t("action.openInFileManager"),
				...
				className: "fe-iconbtn",
				title: t("action.expandCollapseAll"),
				onClick: toggleAll
			}, react.createElement(Icon, {
				name: "chevronDown",
				size: 14
			...
			}), s.searching ? react.createElement("span", { className: "fe-search-state" }, "…") : null), status ? react.createElement("div", { className: "fe-status " + (status.ok ? "fe-status-ok" : "fe-status-err") }, statusText) : null, react.createElement("div", { className: "fe-tree" }, s.query.trim() ? renderSearch() : renderTree())) : null;
